...
- 🎯 Entry points to execute your program using `python -m <moudra>` or `$ moudra` with basic CLI argument parsing.
...
[![codecov](https://codecov.io/gh/matejkratochvil/moudra/branch/main/graph/badge.svg?token=moudra_token_here)](https://codecov.io/gh/matejkratochvil/moudra)
[![CI](https://github.com/matejkratochvil/moudra/actions/workflows/main.yml/badge.svg)](https://github.com/matejkratochvil/moudra/actions/workflows/main.yml)

Awesome moudra created by matejkratochvil

## Install

from source
```bash
git clone https://github.com/matejkratochvil/moudra moudra
cd moudra
make install
```
...
```bash
$ moudra run --port 8080
```

or

```bash
python -m moudra run --port 8080
```
...
```bash
❯ moudra --help
Usage: moudra [OPTIONS] COMMAND [ARGS]...
...
```bash
❯ moudra create-user --help
Usage: moudra create-user [OPTIONS] USERNAME PASSWORD
...
```bash
moudra create-user admin admin --superuser
```
...
Run with `moudra run` and access http://127.0.0.1:8000/docs

![](https://raw.githubusercontent.com/rochacbruno/fastapi-project-template/master/docs/api.png)


**For some api calls you must authenticate** using the user created with `moudra create-user`.
...
-----------------------------------------------------
moudra/__init__.py              4      0   100%
moudra/app.py                  16      1    94%
moudra/cli.py                  21      0   100%
moudra/config.py                5      0   100%
moudra/db.py                   10      0   100%
moudra/models/__init__.py       0      0   100%
moudra/models/content.py       47      1    98%
moudra/routes/__init__.py      11      0   100%
moudra/routes/content.py       52     25    52%
moudra/routes/security.py      15      1    93%
moudra/routes/user.py          52     26    50%
moudra/security.py            103     12    88%
-----------------------------------------------------
TOTAL                               336     66    80%
...
```py
from moudra.config import settings
```
...
> `dynaconf_merge` is a boolean that tells if the settings should be merged with the default settings defined in moudra/default.toml.

### As environment variables
```bash
export moudra_KEY=value
export moudra_KEY="@int 42"
export moudra_KEY="@jinja {{ this.db.uri }}"
export moudra_DB__uri="@jinja {{ this.db.uri | replace('db', 'data') }}"
```
...
```bash
moudra_ENV=production moudra run
```
